...
$limit = 10; 	
 ...
 if($getpage[$countarraypage-2]=='Reviews' && $page>1 ){
 $reviews_word=$getpage[$countarraypage-2];}
if(is_numeric($page)){
 $page = $page;
 $compname=$getpage[0];
 }
...
{	
	...
	//previous button
	if ($page > 1) {
		$pagination.= "<li><a href=\"https://www.topmovingreviews.com/movers/$compname"."_Reviews_"."$prev-$_GET[id]/\">Prev 10</a></li>";
}
	...
	//next button
	if ($page < $counter - 1) 
		$pagination.= "<li><a href=\"https://www.topmovingreviews.com/movers/$compname"."_Reviews_"."$next-$_GET[id]/\">Next 10</a></li>";
	/*else
...
<html lang="en">
<head>
<title><?php echo $res_company['title']."-".$res_company['city'].", ".$res_company['state']." With ".$res_company['count(*)'] ." Moving Reviews ";?></title>
...
<meta name=keywords content="<?php echo $res_company['title'];?>">
   <meta http-equiv="X-UA-Compatible" content="IE=edge" />
      ...
      <meta property=og:title content="<?php echo $res_company['title']."-".$res_company['city'].", ".$res_company['state']." With ".$res_company['count(*)'] ." Moving Reviews ";?>">
...
<meta name="twitter:title" content="<?php echo $res_company['title']."-".$res_company['city'].", ".$res_company['state']." With ".$res_company['count(*)'] ." Moving Reviews ";?>" />
...
<div class="jet jet-1">
    <div class="jeft-newleft">
        ...
        ?>
<div class="col-md-4 text-left"><img src="<?php echo $logo_image;?>"   alt="<?php echo  $res_company['title'];?> - <?php echo $res_company['state'];?> -<?php echo  $res_company['city'];?> - Reviews">
...
if($phonw<>"") {
    if (strpos($phonw, ',') !== false) {
        ...
        $phone2 =  $expar[1]; 
        ?>
    <p><div class=headphone></div> Toll Free:  <?php echo  $phone1;?> | <div class=call></div> Phone:  <?php  echo  $phone2;?>
</p>
    <?php 
    }else{ ?>
        <p><div class=headphone></div> Toll Free:  <?php echo  $phonw;?> | <div class=call></div> Phone:  <?php  echo  $phonw;?>
</p>        
<?php 
    }
...
<br>
<p class="review-para-1" style="font-size:18px !important;">Companies that are offering nationwide services must first register with the Federal Motor Carrier Safety Administration (FMSCA). For more information on the <?php echo  $res_company['title'];?> license numbers for the government record information</p>
<p class="review-para-1" style="font-size:18px !important;">US D.O.T: <a href="<?php echo  $res_dot['company_url'];?>" target="_blank"><?php if($res_dot['usdot_number']==''){ echo "Not Updated"; } else { echo $res_dot['usdot_number'];}?></a><br>
(US Department of Transportation number)</p>
<p class="review-para-1" style="font-size:18px !important;">ICC MC number: <a href="<?php echo  $res_dot['company_url'];?>" target="_blank"><?php if($res_dot['mc']==''){ echo "Not Updated"; } else { echo "MC-".$res_dot['mc'];}?></a><br>
(Interstate Commerce Commission Motor Carrier number)</p>
...
<?php echo  $res_company['title']; if($res_dot['granted_date']<>'') { echo "Start doing business after ". $res_dot['granted_date'].","; } ?> located in <?php echo $res_company['address']; ?>  <a rel="nofollow" href="https://www.topmovingreviews.com/map.php?query=<?php echo substr(str_replace(",","%2C",str_replace(" ","+",$res_company['address'])),0,-4); ?>" target="_blank">View On Map</a>, but moving companies covered larger areas includes states nearby.
</p>

<p class="review-para-1" style="font-size:12px !important;"> 				
        
            <table style="color:black;border:1px solid black;text-align:center;">
                <tr >
                    <th style="border-right:1px solid black;" >Far North East</th><th>North East</th><th>Med East</th><th>South East</th><th>North Med west</th>
                    <th style="border-right:1px solid black;" >South Med west</th>
                    <th style="border-right:1px solid black;" >Far Med West</th>
                    
                    <th style="border-right:1px solid black;" >Far North Med west</th>
                    <th style="border-right:1px solid black;" >North west</th>
                    <th style="border-right:1px solid black;" >Middle North West</th>
                    <th style="border-right:1px solid black;" >West</th>
...
<img src="<?= $logo_small ?>"  alt="" style="width:47px !important; height:25px !important;"></a>
