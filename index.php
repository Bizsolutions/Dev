...
<html lang="en">
    ...
    <body id="shahaidul-body" onmouseover="loadcss" onLoad="doOnLoad();">
        ...
        <div class="content">
            ...
            $result = mysqli_query($connection->conn, $companies);
            $count = 1;
            ...
            foreach ($companeiesId as $id) {
                ...
                $companies_sql = $connection->conn->query($companies_list);
                while ($company = mysqli_fetch_array($companies_sql)) {
//                    echo '<pre>';
//                    print_r($company);die;
                    $company_address = explode(",", $company['address']);
                    $countarray = count($company_address);
                    $state_code = substr(isset($company_address[$countarray - 2]) ? $company_address[$countarray - 2] : '', 1, 2);
                    $sql_state = $connection->conn->query("select name from states where state_code='$state_code' and usa_state=1");
                    $res_state = mysqli_fetch_array($sql_state);

                    $reviews_count = "SELECT count(*) as cnt FROM reviews where company_id = " . $id;
                    $reviews_count_sql = $connection->conn->query($reviews_count);

                    $reviews_list = "SELECT * FROM reviews where company_id = $company[id] order by date_saved desc limit 0,1";
                    $reviews_sql = $connection->conn->query($reviews_list);
                    ...
                    while ($review = mysqli_fetch_array($reviews_sql)) {
                        ...
                        <div class="reviewbox newreviewbox">
                            <div class="review-rating">
                                ...
                                <div class="review-stars">
                                    ...
                                    <p class="review-rating-title"> 
                                        <a href="https://www.topmovingreviews.com/usa/<?= isset($res_state['name']) ? $res_state['name'] : '' . "-movers-" . $state_code; ?>/">
                                            <?= isset($res_state['name']) ? $res_state['name'] : ''; ?> 
                                        </a> 
                                        <i>moving</i> 
                                        company
                                    </p>
                            ...
                            <div class="review-by">
                                ...
                                &ensp; | <?php
                                if (strlen($review['move_location_text']) > 0) {
                                    echo $review['move_location_text'];
                                }
                                ?>
        ...
        <div class="fooo">
            <div id="footer_popular_cities" class="well well-lg center-block">
                ...
                <?php /* ?><div class="footer_link">
                  ...
                  $query_state_cocde=mysql_query($sql_state_code);
                  //$row=1;
                  while($res_state_code=mysql_fetch_array($query_state_cocde))
                  {
