...
class company {
	...
	function delete_company($company_id) {
		...
		if($req = mysqli_query($link,$str)) { $this->delete_related_reviews($company_id); return true; }
		else return false;
	}
   ...
   function select_company($id) {
	...
	function get_company_reviews_count($company_id,$link) {
		$str = "SELECT * FROM reviews WHERE Company_ID = $company_id";
		$req = mysqli_query($link,$str);
		$num = mysqli_num_rows($req);
		return $num;
	}
		function get_average_reviews_count($company_id,$link) {
		$str = "SELECT avg(rating) FROM reviews WHERE Company_ID = $company_id";
		$rat = mysqli_query($link,$str);
		$rating = mysqli_fetch_assoc($rat);
		return $rating;
	}
