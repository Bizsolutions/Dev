...
$check = mysqli_query($db, "select id, mc_mx_ff_url from company_dot_data_sri where operating_state LIKE '%HHG%' AND mc_mx_ff_url != '' and mc_mx_ff_check = '0' ") or die(mysqli_error($db));
$row = mysqli_fetch_assoc($check);
echo "Left ##### " . mysqli_num_rows($check)."\n\n\n\n\n";

do{
    $check = mysqli_query($db, "select id, mc_mx_ff_url from company_dot_data_sri where operating_state LIKE '%HHG%' AND mc_mx_ff_url != '' and mc_mx_ff_check = '0'  LIMIT 50 OFFSET ".$offset) or die(mysqli_error($db));
    $row = mysqli_fetch_assoc($check);
   ...
   //   }while( strlen($g_recaptcha_response) < 111 ); //"CAPCHA_NOT_READY"
        ...
        foreach ($dom->query('//table[@summary="table used for formating purposes only"][3]/tr') as $td) { 
            ...
            if(!empty(array_filter($d)) ){
                ...
                $query = "INSERT INTO `company_dot_authority_data_sri` (".$col.") VALUES ($val)";
                mysqli_query($db, $query) or die(mysqli_error($db));

                $query = "UPDATE  `company_dot_data_sri` SET  `mc_mx_ff_check` = '1' WHERE  `id` = '$row[id]' ;";
                mysqli_query($db, $query) or die(mysqli_error($db));
