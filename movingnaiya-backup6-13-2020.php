...
<?=$pagination?>
<?php 
$sql_dot="select power_units,usdot_number,mc,safety_url,date_format(granted_date,\"%m-%d-%Y\") as granted_date from company_dot_data where  company_name= '$res_company[title]' and state='$res_company[state]' and city='$res_company[city]'";
...
<br>
<p class="review-para-1" style="font-size:18px !important;">Companies that are offering nationwide services must first register with the Federal Motor Carrier Safety Administration (FMSCA). For more information on the Accurate Express Movers license numbers for the government record information</p>
<p class="review-para-1" style="font-size:18px !important;">US D.O.T: <a href=""><?php if($res_dot['usdot_number']==''){ echo "Not Available"; } else { echo $res_dot['usdot_number'];}?></a><br>
(US Department of Transportation number)</p>
<p class="review-para-1" style="font-size:18px !important;">ICC MC number: <a href="">MC-<?php if($res_dot['mc']==''){ echo "Not Available"; } else { echo $res_dot['mc'];}?></a><br>
...
<?php echo  $res_company['title'];?> Start doing business after June 10, 2008, located in <?php echo $res_company['address']; ?>  <a rel="nofollow" href="https://www.topmovingreviews.com/map.php?query=<?php echo substr(str_replace(",","%2C",str_replace(" ","+",$res_company['address'])),0,-4); ?>" target="_blank">View On Map</a>, but moving companies covered larger areas includes states nearby.
</p>
