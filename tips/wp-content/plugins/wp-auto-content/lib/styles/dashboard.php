...
if ( !class_exists( 'ASDashboard' ) ) {
		...
		function show_dashboard_widget() {
			...
			if ( !isset($items) )
			     foreach ( $rss->get_items(0, $this->num_items) as $item ) {
			          ...
			          $content .= "<p style='text-align:right'><a href='$link' class='button button-secondary'  target='_blank'>Download Now</a></p>";
			...
			$str .= '</ul><div style="clear:both"></div><a href="https://wpautocontent.com/support/" class="button button-secondary" target="_blank">? Support</a> <a href="http://licensemachine.com/access/other-products.php" class="button button-primary" target="_blank">+ View More</a><a style="float:right" href="#" id="wp_asdashboard_hide">Don\'t show this widget</a><div style="clear:both"></div>';
	...
	$as_dashboard = new ASDashboard( 'wpautcnt' );
}

if ( !class_exists( 'ASDashboardPlugins' ) ) {
	class ASDashboardPlugins {
		var $plugin_name;
		var $feed = 'https://kreatelinks.com/ourproducts/feed/?t=12';
		var $num_items = 35;
		var $quick = 0;
		function __construct( $plugin_name ) {
			$this->plugin_name = $plugin_name;
		...
		private function do_get_remote_plugins() {
			...
			foreach ( $rss->get_items() as $item ) {
				...
				$purchaseurl = add_query_arg( 'tid', 'addpl_'.$this->plugin_name, $purchaseurl );
		...
		function plugin_links( $links, $plugin ) {
			if( isset( $plugin[ 'pltypea' ] ) && ( $plugin[ 'pltypea' ] == 'ankur' ) ) {
	...
	$as_dashboard = new ASDashboardPlugins( 'autocontent' );
}
?>