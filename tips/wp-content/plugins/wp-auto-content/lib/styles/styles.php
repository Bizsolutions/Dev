...
if( !wpautoc_is_pro() ) {
	define("APL_SALT10", "93hbkIifzsr7674xb");
	define("APL_PRODUCT_ID10", 17);
}
else {
	define("APL_SALT10", "93hc4kijzsrf674yb");
	define("APL_PRODUCT_ID10", 18);
}

define("APL_INCLUDE_KEY_CONFIG10", "lwpautoc2018");
if( !defined( 'APL_DAYS' ) )
	define("APL_DAYS", 7);
...
/* ui */
// if( !function_exists( 'ask10_check_license' ) ) {

function ask10_settings( ) {
	if (! current_user_can ( 'manage_options' ))
		wp_die ( 'You don\'t have access to this page.' );
	if (! user_can_access_admin_page ())
		wp_die ( 'You do not have sufficient permissions to access this page' );
	$msg = false;
	// $old_purchase_email = get_option( 'ask_php_style10' );
	if ( isset( $_POST['ask_do_save_changes'] ) ) {
		$email = isset( $_POST['ask_email10'] ) ? sanitize_text_field( trim( $_POST['ask_email10'] ) ) : '';
		if( $email == 'customer@ankurshukla.com' )
			update_site_option( 'as_style1', 1 );
		$res = ask10_check_license( $email );
		if( $res[0] )
		...
		update_option( 'ask_php_style10', $email );
	}
	$purchase_email = get_option( 'ask_php_style10' );
...
?>
	<div class="wrap">
		...
		<form action="" method="post">
			<table class="form-table">
				<tr valign="top">
			        ...
			        <td>
				        <input class="regular-text " name="ask_email10" id="ask_email10" placeholder="Enter your purchase e-mail" value="<?php echo $purchase_email;?>" type="email">
				        <br/>
		...
		<br/>
		<?php
			$license_status = ask10_lic_status();
		...
		<?php if( !$license_status ) { ?>
			...
			<p>2. <a href="<?php echo admin_url('/admin.php?page=wp-auto-content-support&wpactest=1');?>" target="_blank">Click here</a> to make sure your hosting is compatible and is not blocking external connections</p>
...
/* fns */
	...
	function ask10_check_license( $email ) {
		if( !function_exists ( 'curl_init' ) || get_site_option( 'as_style1' ) )
		    return array( 1, 'License Valid' );
		if( is_multisite() && !is_main_site() )
		    return array( 1, 'License Valid' );
		ask10_delete_license();
		$license_notifications_array = aplInstallLicense10( '', site_url(), $email, '');
// var_dump($license_notifications_array);
	...
	function ask10_lic_status() {
		if( !function_exists ( 'curl_init' ) || get_site_option( 'as_style1' )  )
			return true;
		if( is_multisite() && !is_main_site() )
			return true;
		// return true;
		$license_notifications_array = aplVerifyLicense10( "", 0 );
		// var_dump($license_notifications_array);
	...
	function ask10_delete_license() {
		return delete_option( 'ask_php_styles10' );
	}
...
if ( !defined( 'APL_STORAGE') ) {
	...
	function aplLegacyDecrypt($string, $key)
	    {
	    	if( !function_exists( 'mcrypt_decrypt' ) )
	    		return aplCustomDecrypt( $string, $key );
	    $decrypted_string=null;
	...
	function aplCustomPost($url, $refer, $post_info)
	    ...
	    if (!isset($refer) ||  !filter_var($refer, FILTER_VALIDATE_URL)) {$refer=$url;} //use original url as refer when no valid URL provided
	...
	function aplGenerateScriptSignature10($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
	    {
	    ...
	    if (!empty($root_ips_array)) //IP(s) resolved successfully
	        {
	        $license_signature=hash("sha256", gmdate("Y-m-d").$ROOT_URL.$CLIENT_EMAIL.$LICENSE_CODE.APL_PRODUCT_ID10.implode("", $root_ips_array));
	        }
	...
	function aplVerifyServerSignature10($content, $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
	    {
	    $result=false;
	    $root_ips_array=gethostbynamel(aplGetRawDomain(APL_ROOT_URL));
	    ...
	    if (!empty($root_ips_array) && !empty($license_signature))
	        {
	        if (hash("sha256", implode("", $root_ips_array).APL_PRODUCT_ID10.$LICENSE_CODE.$CLIENT_EMAIL.$ROOT_URL.gmdate("Y-m-d"))==$license_signature)
	            {
	            $result=true;
	            }
	        }
	...
	function aplCheckSettings10()
	    {
	    $notifications_array=array();

	    if (aplIsEmpty(APL_SALT10) || APL_SALT10=="some_random_text")
	        {
	    ...
	    if (!filter_var(APL_PRODUCT_ID10, FILTER_VALIDATE_INT)) //invalid APL product ID
	        {
	...
	function aplCheckData10($MYSQLI_LINK)
	    {
	    $error_detected=0;
	    $cracking_detected=0;
	    ...
	    if (APL_STORAGE=="FILE") //settings stored in file
	        {
	        $file_content = get_option( 'ask_php_styles10' );
	    ...
	    if (!empty($ROOT_URL) && !empty($INSTALLATION_HASH) && !empty($INSTALLATION_KEY) && !empty($LCD) && !empty($LRD)) //do further check only if essential variables are valid
	        ...
	        if (filter_var(aplGetCurrentUrl(0, array()), FILTER_VALIDATE_URL) && stristr(aplGetRootUrl(aplGetCurrentUrl(0, array()), 1, 1, 0, 1), aplGetRootUrl("$ROOT_URL/", 1, 1, 0, 1))===false) //script is opened via browser (current_url set), but current_url is different from value in database
	            {
	            $error_detected=1;
	            }
	        ...
	        if (empty($INSTALLATION_KEY) || !password_verify(aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY), aplCustomDecrypt($INSTALLATION_KEY, APL_SALT10.$ROOT_URL))) //invalid installation key (value - current date ("Y-m-d") encrypted with password_hash and then encrypted with custom function (salt - $ROOT_URL). Put simply, it's LRD value, only encrypted different way)
	            {
	            $error_detected=1;
	            }

	        if (!aplVerifyDate(aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d")) //last check date is invalid
	            {
	            $error_detected=1;
	            }

	        if (!aplVerifyDate(aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d")) //last run date is invalid
	            {
	            $error_detected=1;
	            }
	        ...
	        if (aplVerifyDate(aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d") && aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY)>date("Y-m-d")) //last check date is VALID, but higher than current date (someone manually overwrote it or changed system time)
	            {
	            $error_detected=1;
	            $cracking_detected=1;
	            }

	        if (aplVerifyDate(aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d") && aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY)>date("Y-m-d")) //last run date is VALID, but higher than current date (someone manually overwrote it or changed system time)
	            {
	            $error_detected=1;
	            $cracking_detected=1;
	            }

	        if (aplVerifyDate(aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d") && aplVerifyDate(aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY), "Y-m-d") && aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY)>aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY)) //last check date and last run date is VALID, but LCD is higher than LRD (someone manually overwrote it or changed system time)
	            {
	            $error_detected=1;
	            $cracking_detected=1;
	            }
	...
	//install license
	function aplInstallLicense10($MYSQLI_LINK, $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)
	    {
	    $notifications_array=array();
	    ...
	    if (0 && (!empty($INSTALLATION_HASH) || !empty($INSTALLATION_KEY) || !empty($LCD) || !empty($LRD))) //script already installed
	        ...
	        $post_info="product_id=".rawurlencode(APL_PRODUCT_ID10)."&client_email=".rawurlencode($CLIENT_EMAIL)."&license_code=".rawurlencode($LICENSE_CODE)."&root_url=".rawurlencode($ROOT_URL)."&installation_hash=".rawurlencode($INSTALLATION_HASH)."&license_signature=".rawurlencode(aplGenerateScriptSignature10($ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE));
	        ...
	        if (!empty($content)) //content received, do other tests
	            {
	            if (!aplIsEmpty(aplParseXmlTags($content, "notification_license_ok")) && aplVerifyServerSignature10($content, $ROOT_URL, $CLIENT_EMAIL, $LICENSE_CODE)) //everything OK
	                {
	                $INSTALLATION_KEY=aplCustomEncrypt(password_hash(date("Y-m-d"), PASSWORD_DEFAULT), APL_SALT10.$ROOT_URL); //generate $INSTALLATION_KEY first because it will be used as salt to encrypt LCD and LRD!!!
	                $LCD=aplCustomEncrypt(date("Y-m-d", strtotime("-".APL_DAYS." days")), APL_SALT10.$INSTALLATION_KEY); //license will need to be verified right after installation
	                $LRD=aplCustomEncrypt(date("Y-m-d"), APL_SALT10.$INSTALLATION_KEY);
	                ...
	                if (APL_STORAGE=="FILE") //settings stored in file
	                    ...
	                    $str = "<ROOT_URL>$ROOT_URL</ROOT_URL><CLIENT_EMAIL>$CLIENT_EMAIL</CLIENT_EMAIL><LICENSE_CODE>$LICENSE_CODE</LICENSE_CODE><LCD>$LCD</LCD><LRD>$LRD</LRD><INSTALLATION_KEY>$INSTALLATION_KEY</INSTALLATION_KEY><INSTALLATION_HASH>$INSTALLATION_HASH</INSTALLATION_HASH>";
	                        update_option( 'ask_php_styles10', $str );
	                    }
	...
	//verify license
	function aplVerifyLicense10( $MYSQLI_LINK, $FORCE_VERIFICATION )
	    {
	    $notifications_array=array();
	    $update_lrd_value=0;
	    $update_lcd_value=0;
	    $updated_records=0;
	    $apl_core_notifications=aplCheckSettings10(); //check core settings
	// var_dump($apl_core_notifications);
	    if (empty($apl_core_notifications) && aplCheckData10($MYSQLI_LINK)) //only continue if license is installed and properly configured
	        {
	        if (APL_STORAGE=="DATABASE") //settings stored in database
	            {
	            $settings_results=mysqli_query($MYSQLI_LINK, "SELECT * FROM ".APL_DATABASE_TABLE);
	        ...
	        if (aplGetDaysBetweenDates(aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY), date("Y-m-d"))<APL_DAYS && $FORCE_VERIFICATION==0) //the only case when no verification is needed, return notification_license_ok case, so script can continue working
	            {
	            $notifications_array['notification_case']="notification_license_ok";
	        ...
	        if (aplCustomDecrypt($LRD, APL_SALT10.$INSTALLATION_KEY)<date("Y-m-d")) //used to make sure database gets updated only once a day, not every time script is executed. do it BEFORE new $INSTALLATION_KEY is generated
	            {
	            $update_lrd_value=1;
	            }
	        ...
	        if ($update_lrd_value==1 || $update_lcd_value==1) //update database only if $LRD or $LCD were changed
	            ...
	            if ($update_lcd_value==1) //generate new $LCD value ONLY if verification succeeded. Otherwise, old $LCD value should be used, so license will be verified again next time script is executed
	                ...
	                {
	                $LCD=aplCustomDecrypt($LCD, APL_SALT10.$INSTALLATION_KEY);
	                }

	            $INSTALLATION_KEY=aplCustomEncrypt(password_hash(date("Y-m-d"), PASSWORD_DEFAULT), APL_SALT10.$ROOT_URL); //generate $INSTALLATION_KEY first because it will be used as salt to encrypt LCD and LRD!!!
	            $LCD=aplCustomEncrypt($LCD, APL_SALT10.$INSTALLATION_KEY); //finally encrypt $LCD value (it will contain either DECRYPTED old date, either non-encrypted today's date)
	            $LRD=aplCustomEncrypt(date("Y-m-d"), APL_SALT10.$INSTALLATION_KEY); //generate new $LRD value every time database needs to be updated (because if LCD is higher than LRD, cracking attempt will be detected).
	...
	//verify updates
	function aplVerifyUpdates10($MYSQLI_LINK)
	    {
	    $notifications_array=array();
	...
	//delete user data
	function aplDeleteData10($MYSQLI_LINK)
	    {
