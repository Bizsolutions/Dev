...
function wpautoc_traffic_facebook( $post_id, $traffic, $settings ) {
	// echo "entro bmachine";
	/*var_dump($settings);*/
	if( empty( $settings ) )
		return false;
	...
	if( empty( $fb_page ) ) {
		wpautoc_debug( 'Error with Facebook - No Page Selected', 'error' );
		return;
	}
	// var_dump($post_id);
...
// return;
	$res = wpautoc_share_facebook( $fb_page, get_the_title( $post_id ), /*get_the_excerpt( $post_id )*/'', get_permalink( $post_id ) );
	if( $res )
		wpautoc_log_traffic( $post_id, WPAUTOC_TRAFFIC_FACEBOOK );
}

function wpautoc_share_facebook( $fb_page, $title, $text, $post_url, $image_url = false ) {
...
// var_dump($facebook_settings);
	...
	if ( ! isset( $facebook_settings['app_id'] ) || empty( $facebook_settings['app_id'] ) || ! isset( $facebook_settings['app_secret'] ) || empty( $facebook_settings['app_secret'] ) ) {
		wpautoc_debug( 'Error with Facebook Credentials', 'error' );
		return false;
	}
    $pageAccessToken = wpautoc_page_access_token( $fb_page );

	// $fbpageid             = $fb_page;
	$data['link']         = $post_url;
	$data['message']      = html_entity_decode( $title );

	wpautoc_facebook_post( $fb_page . '/feed', $data, $pageAccessToken );
	// var_dump($post_url);
	// trfInsertHistory('facebook', json_encode($return), $link, $title, $post_id);
	return true;
}

function wpautoc_get_facebook_pages() {
	require_once WPAUTOC_DIR . 'lib/libs/facebook/Facebook/autoload.php';

	$response = wpautoc_facebook_query( "me/accounts", "" );
	$pages_array = array();
...
function wpautoc_facebook_query( $query, $params ) {
...
function wpautoc_facebook_post ( $query, $params, $pagetoken = false ) {
    $post_url = 'https://graph.facebook.com/'.$query;
...
function wpautoc_page_access_token( $pageid ) {
  $response = wpautoc_facebook_query('me/accounts', '');
