...
class YTYoutube
{
    ...
    //order in search api
    const ORDER_DATE = YTConstants::ORDER_DATE;
    const ORDER_RATING = YTConstants::ORDER_RATING;
    const ORDER_RELEVANCE = YTConstants::ORDER_RELEVANCE;
    const ORDER_TITLE = YTConstants::ORDER_TITLE;
    const ORDER_VIDEOCOUNT = YTConstants::ORDER_VIDEOCOUNT;
    const ORDER_VIEWCOUNT = YTConstants::ORDER_VIEWCOUNT;

    //eventType
    const EVENT_TYPE_LIVE = YTConstants::EVENT_TYPE_LIVE;
    const EVENT_TYPE_COMPLETED = YTConstants::EVENT_TYPE_COMPLETED;
    const EVENT_TYPE_UPCOMING = YTConstants::EVENT_TYPE_UPCOMING;

    //type in search api
    const SEARCH_TYPE_CHANNEL = YTConstants::SEARCH_TYPE_CHANNEL;
    const SEARCH_TYPE_PLAYLIST = YTConstants::SEARCH_TYPE_PLAYLIST;
    const SEARCH_TYPE_VIDEO = YTConstants::SEARCH_TYPE_VIDEO;
     ...
     */
    public function searchVideos( $q, $maxResults = 10, $order = null, $extra_params = false )
    {
        $params = array(
            'q' => $q,
            'type' => 'video',
     ...
     * @return object
     */
    public function searchChannelVideos($q, $channelId, $maxResults = 50, $order = null, $extra_params = false, $pageInfo = false )
    {
        $params = array(
            'q' => $q,
            'type' => 'video',
     ...
     * @return \StdClass  API results
     */
    public function searchTrends($country, $maxResults = 50, $order = null, $extra_params = false, $pageInfo = false)
    {
        $API_URL = $this->getApi('videos.list');
        $params = array();
        if (!empty($country)) {
    ...
    public function api_get($url, $params)
    {
        ...
        $tuCurl = curl_init();
        if ( defined( 'VIDPROFITS_AI_CRON_DEBUG' ) )
            echo "<p>Youtube URL: ".$url . (strpos($url, '?') === false ? '?' : '') . http_build_query($params)."</p>";
