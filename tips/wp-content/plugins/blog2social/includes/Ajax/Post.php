        ...
        if (isset($_POST['b2s_security_nonce']) && (int) wp_verify_nonce($_POST['b2s_security_nonce'], 'b2s_security_nonce') > 0) {
            if (isset($_POST['postId']) && !empty($_POST['postId'])) {
                ...
                if (is_array($postIds) && !empty($postIds)) {
                    ...
                    global $wpdb;
                    $sql = $wpdb->prepare("SELECT id FROM {$wpdb->prefix}b2s_posts WHERE sched_type = 5 AND hide = 0 AND publish_date = '0000-00-00 00:00:00' AND blog_user_id = %d AND post_id IN ($postIdsString)", B2S_PLUGIN_BLOG_USER_ID);
                    $result = $wpdb->get_results($sql);
                    if(is_array($result) && !empty($result)) {
                        $b2sPostIds = array();
                        foreach ($result as $k => $v) {
