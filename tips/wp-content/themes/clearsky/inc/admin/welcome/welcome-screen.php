...
class IGthemes_Welcome {
   ...
   public function igthemes_welcome_admin_notice() {
    ...
    <div class="updated notice is-dismissible">
        <p>
            <?php echo esc_html__( 'Thanks for choosing ', 'clearsky' ). wp_get_theme()->get( 'Name' );
            echo sprintf( esc_html__('! You can read documentation to how get the most out of your new theme on the %swelcome screen%s.', 'clearsky' ), '<a href="' . esc_url( admin_url( 'themes.php?page=igthemes-welcome' ) ) . '">', '</a>' ); ?>
        </p>
        <p>
            <a href="<?php echo esc_url( admin_url( 'themes.php?page=igthemes-welcome' ) ); ?>" class="button button-primary" style="text-decoration: none;">
                <?php echo sprintf( esc_html__( 'Get started with %s', 'clearsky' ), wp_get_theme()->get( "Name" ) ); ?>
            </a>
        </p>
    </div>
        <?php
