...
function igthemes_posts_navigation() {
    ...
    if (get_theme_mod('numeric_pagination')) {
        ...
        echo paginate_links( array(
            ...
            'type'    => 'list',
            'prev_next'    => True,
            'prev_text'    => esc_html__('&#8592;', 'clearsky'),
            'next_text'    => esc_html__('&#8594;', 'clearsky'),
    ...
    <?php }// end pagination
        ...
        the_posts_navigation(array(
            'prev_text'          => __( 'Previous','clearsky' ),
            'next_text'          => __( 'Next','clearsky' ),
        ));
    }
    echo "</div>";
}
...
function igthemes_posted_on() {
    //AUTHOR
    $byline = sprintf(
          esc_html_x( apply_filters('igthemes-author','igthemes-author-text') . '%s', 'post author', 'clearsky' ),
	...
	$posted_on = sprintf(
		esc_html_x( apply_filters('igthemes_date', 'igthemes_date_text') . '%s', 'post date', 'clearsky' ),
...
function igthemes_entry_footer() {
	...
	if ( 'post' === get_post_type() && apply_filters( 'igthemes_post_cat', true ) ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'clearsky' ) );
		if ( $categories_list && igthemes_categorized_blog() && apply_filters( 'igthemes_post_cat', true ) ) {
			printf( '<span class="cat-links">' . esc_html__( apply_filters('igthemes_cat_links', 'igthemes-cat-text') . '%1$s', 'clearsky' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'clearsky' ) );
		if ( $tags_list && apply_filters( 'igthemes_post_tags', true ) ) {
			printf( '<span class="tags-links">' . esc_html__( apply_filters('igthemes_tags_links', 'igthemes-tags-text') . '%1$s', 'clearsky' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
    //commnet
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) && apply_filters( 'igthemes_post_comments_link', true ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'clearsky' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
    //edit link
    edit_post_link(
