...
<script type="application/ld+json">
            {
                ...
                "review": {<?php $i = 0 ;  while ($res_reviews22 = mysql_fetch_array($resultsllrl)) {   if (strlen($res_reviews22['move_location_text']) > 0) { $move_loca_text  = $res_reviews22['move_location_text']; } ?>
                    {
                        "@type": "Review",
                            "author": "<?php echo $res_reviews22['author'];?>",
                            "datePublished": "<?php echo date('M d,Y', strtotime($res_reviews22['date'])); ?>",
                            "name": "<?php echo substr($move_loca_text, 0 , 150);?>",
                            "description": "<?php if (strlen($res_reviews22['move_location_text']) > 0) { echo $res_reviews22['move_location_text']; } ?>",
                            "reviewRating": {
                                "@type": "Rating",
                                "bestRating": "<?php echo $bestRating;?>",
                                "worstRating": "<?php echo $minRating;?>",
                                "ratingValue": "<?php echo round($res_reviews["rating"]); ?>"
                            }
                    } <?php $i++; if( $i!= $nnresult) echo ','; ?> <?php  } ?>
                }
            }
</script>
<?php } ?>
